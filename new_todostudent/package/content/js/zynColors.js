/*
 *  Colors Set By Soft's
 * 	2019 Zyntelligent Co., Ltd.
 *  updated on 27/05/2563
 */
var zynColors = (function() {

	var md = {
		getArray : function(num,pos = this.length) {
			let array = [];
			for ( var i in this) {
				if(i == 'getArray') continue;
				else if (this.hasOwnProperty(i)) {
					if(this[i][num.toString()]){
						array.push(this[i][num.toString()]);
					}
				}
			}
			return array.slice(0, pos);
		},
		"red" : {
			"50" : "#ffebee",
			"100" : "#ffcdd2",
	...
	var colorArray = [ "#ef5350", "#ec407a", "#ab47bc", "#7e57c2", "#5c6bc0",
...
/*	
	for ( var i in md) {
		if (md.hasOwnProperty(i)) {
			colorArray.push(md[i]['400']);
		}
	}
	for ( var i in md) {
		if (md.hasOwnProperty(i)) {
			colorArray.push(md[i]['a200']);
		}
	}
	for ( var i in md) {
		if (md.hasOwnProperty(i)) {
			colorArray.push(md[i]['a700']);
		}
	}
	for ( var i in md) {
		if (md.hasOwnProperty(i)) {
			colorArray.push(md[i]['600']);
		}
	}
	for ( var i in md) {
		if (md.hasOwnProperty(i)) {
			colorArray.push(md[i]['800']);
		}
	}
	for ( var i in md) {
		if (md.hasOwnProperty(i)) {
			colorArray.push(md[i]['900']);
		}
	}
	for ( var i in md) {
		if (md.hasOwnProperty(i)) {
			colorArray.push(md[i]['200']);
		}
	}
	console.log(JSON.stringify(colorArray.filter(n => n) ) ); */
	return {
